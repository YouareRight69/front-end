...
export default function BarChart1() {
    ...
    const loadChart = async () => {
        const result = await axios.get("http://localhost:8080/api/admin/chart/chart")
        setChart(result.data)
    }

    const loadUser = async () => {
        const result = await axios.get("http://localhost:8080/api/admin/chart/totalUser")
        setUser(result.data[0].totalUser)
        console.log(result.data)
    }

    const loadTotal = async () => {
        const result = await axios.get("http://localhost:8080/api/admin/chart/total")
        setTotal(result.data[0].total)

    }

    const loadLimit = async () => {
        const result = await axios.get("http://localhost:8080/api/admin/chart/limmit")
            ...
            <div className='row'>
                ...
                <div className="col-10 pt-5" >
                    ...
                    <div className='d-flex pt-5'>
                        <div className='col-4'>
                            <div>
                                <div className="info-box shadow-lg primary" style={{ borderRadius: "10px", padding: '20px', width: '250px' }} >
                                    ...
                                    <div className='' style={{ display: 'flex' }}>
                                        ...
                                        <div className="info-box-icon " style={{ marginLeft: '40px', display: 'flex', justifyContent: 'center', alignItems: 'center', background: 'orange', borderRadius: '50%', width: '60px' }} >
                            ...
                            <div>
                                <div className="info-box shadow-lg primary" style={{ borderRadius: "10px", padding: '20px', width: '250px', height: '127.98px' }} >
                                    <div className='' style={{ display: 'flex' }}>
                                        ...
                                        <div className="info-box-icon " style={{ marginLeft: '40px', display: 'flex', justifyContent: 'center', alignItems: 'center', background: '#11b981', borderRadius: '50%', width: '60px' }} >
                                            <FontAwesomeIcon icon={faDollar} size="xl" />
                                        </div>
                                    </div>
                                    <div style={{ marginTop: '20px' }}>
                                        <div>
                                            <h4 style={{ display: 'inline', color: 'red' }}>&#9660; 16% </h4>
                                            <span style={{ display: 'inline' }}>KỂ TỪ THÁNG TRƯỚC</span>
                                        </div>
                                    ...
                                    <table class="table">
                                        ...
                                        <tbody>
                                            {limit.map((staff, index) => (
                                                <tr>
                                                    <th scope="row" key={index}>{staff.user_id}</th>
